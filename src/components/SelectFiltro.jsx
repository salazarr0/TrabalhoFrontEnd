...
function SelectFiltro(props) {
  ...
  useEffect(() => {
    ...
    const filtrados = todosOsItens.filter((item) => {
      const matchCategoria = ctg === "Todos" || item.categoria === ctg;
      const matchFiliacao = filiacoes.length === 0 || filiacoes.includes(item.filiacao);
      const matchNivel = niveis.length === 0 || (item.nivel && item.nivel.some((n) => niveis.includes(n)));

      return matchCategoria && matchFiliacao && matchNivel;
    });

    props.setShinobiDex(filtrados);
  }, [filiacoes, niveis]); 
      ...
      <div className="flex gap-2 flex-wrap justify-center items-center">
        ...
        {opcoesFiliacao.map((f) => (
          <button
            ...
            className={`px-2 py-1 text-[10px] font-bold uppercase rounded-md border-2 transition-colors ${
              filiacoes.includes(f) ? "bg-orange-400 border-orange-400 text-gray-900" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-orange-400"
            }`}
          >
            {f}
          </button>
      ...
      <div className="flex gap-2 flex-wrap justify-center items-center">
        ...
        {opcoesNivel.map((n) => (
          <button
            ...
            className={`px-2 py-1 text-[10px] font-bold uppercase rounded-md border-2 transition-colors ${
              niveis.includes(n) ? "bg-green-500 border-green-500 text-white" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-green-500"
            }`}
          >
            {n}
          </button>
